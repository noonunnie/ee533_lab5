...
// // 2/17/26 LAB5 UPDATE: commented out ids wires/regs and instatiated pipeline wires/regs
   //------- ids wires/regs ------
/* 
...
*/
  ...
  // 2/17/26 LAB5 UPDATE: only changed all instances of ids to pipeline in following 5 lines
   output_port_lookup
     ...
     .reg_src_in           (op_lut_in_reg_src),
      ...
      // 2/17/26 LAB5 UPDATE: changed all instances of ids to pipeline in following 6 lines
...
/* // 2/17/26 LAB5 UPDATE: We comment IDS out and replace it with our pipeline design to test it independently in lab 5
   //--------------------------------------------------
   // IDS (Intrusion Detection System) STAGE
   // Packet inspection using sliding window to detect bad sequences and drop if match is found
   //--------------------------------------------------
...
// // 2/17/26 LAB5 UPDATE: The new stage for lab 5 that has a 4 stage pipeline using BRAM blocks 
// for Instruction Memory and Data Memory and 1 dummy stage that we will use to implement the ALU later
   //--------------------------------------------------
   // PIPELINE STAGE
   // User-defined module for pipelining to use Instructions from IMemoryInterface (BRAM block)
   // to read values from DMemoryInterface and copy into a different location in DMemoryInterface
   //--------------------------------------------------
   pipeline #(
      .DATA_WIDTH(DATA_WIDTH),
      .CTRL_WIDTH(CTRL_WIDTH),
